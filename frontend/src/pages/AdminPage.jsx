  ...
  return (
    <main className="max-w-7xl mx-auto px-6 py-10">
      ...
      <div className="flex items-center justify-between mb-10">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#6c63ff] to-[#a855f7] flex items-center justify-center">
          ...
          <p className="text-[#8896ab]">Manage your pet inventory — create, update, or remove listings.</p>
        </div>
        <button
          onClick={openAdd}
          className="flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-white bg-gradient-to-r from-[#6c63ff] to-[#a855f7] hover:opacity-90 hover:scale-105 transition-all shadow-lg shadow-[#6c63ff]/30"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        {[
          { label:'Total Pets',  value: pets.length,                              color:'#6c63ff' },
          ...
          <div key={s.label} className="glass p-4">
            <p className="text-[#8896ab] text-xs font-medium uppercase tracking-wider">{s.label}</p>
      ...
      {loading ? (
        ...
        <div className="glass flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-[#8896ab] text-lg mb-4">No pets yet.</p>
          <button onClick={openAdd} className="px-6 py-2.5 rounded-xl bg-[#6c63ff] text-white font-semibold hover:opacity-90 transition">
            Add First Pet
          </button>
        </div>
      ) : (
      ...
      <Dialog open={!!deleteId} onClose={() => setDeleteId(null)}
        PaperProps={{ sx:{ background:'#161b26', border:'1px solid #2a3247', borderRadius:2 } }}>
        <DialogTitle sx={{ fontFamily:"'Space Grotesk',sans-serif", fontWeight:700 }}>
          ⚠️ Delete Pet?
        </DialogTitle>
        <DialogContent>
          <p className="text-[#8896ab]">This action is permanent and cannot be undone.</p>
        </DialogContent>
        <DialogActions sx={{ p:2, gap:1 }}>
          <Button onClick={() => setDeleteId(null)} variant="outlined" color="inherit" sx={{ borderColor:'#2a3247' }}>
            Cancel
          </Button>
